...
const getProductWithFilter = async (
    ...
    if (target) {
        const targetInput = factory.split(",");
        whereClause.target = {
            in: targetInput,
        };
    }
    ...
    if (price) {
        const pricetInput = factory.split(",");

        const priceCondition = [];

        for (let i = 0; i < pricetInput.length; i++) {
            if (pricetInput[i] === "duoi-10-trieu") {
                priceCondition.push({ price: { lt: 10000000 } });
            }
            if (pricetInput[i] === "10-15-trieu") {
                priceCondition.push({ price: { gte: 10000000, lte: 15000000 } });
            }
            if (pricetInput[i] === "15-20-trieu") {
                priceCondition.push({ price: { gte: 15000000, lte: 20000000 } });
            }
            if (pricetInput[i] === "tren-20-trieu") {
...
export { userFilter, yeuCau1, yeuCau2, yeuCau3, yeuCau4, getProductWithFilter };
